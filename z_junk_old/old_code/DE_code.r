...
# # # 5. Convert results to a clean, sortable data frame
# # de_results_df <- as.data.frame(res_enrichment) %>%
# #   trible::rownames_to_column(var = "gene_id") %>%
# #   arrange(pvalue) # Puts the most statistically significant genes at the top

# # # 6. Save your final target list to a CSV file on your Google Drive
# # write.csv(de_results_df, file = "pulldown_enrichment_results.csv", row.names = FALSE)


# # # 1. Add a labeling column to categorize genes for coloring
# # plot_data <- de_results_df %>%
# #   mutate(
# #     threshold = case_when(
# #       log2FoldChange > 1 & padj < 0.05  ~ "Enriched Target",
# #       log2FoldChange < -1 & padj < 0.05 ~ "Depleted",
# #       TRUE                               ~ "Not Significant"
# #     )
# #   )

# # # 2. Build the volcano plot
# # ggplot(plot_data, aes(x = log2FoldChange, y = -log10(padj))) +
# #   # Background/non-significant genes (smaller, semi-transparent)
# #   geom_point(data = filter(plot_data, threshold == "Not Significant"), 
# #              color = "grey80", alpha = 0.5, size = 1.5) +
# #   # Depleted genes (rare in pulldowns, but good to see)
# #   geom_point(data = filter(plot_data, threshold == "Depleted"), 
# #              color = "royalblue", alpha = 0.6, size = 2) +
# #   # True enriched targets (larger, vibrant color to pop out)
# #   geom_point(data = filter(plot_data, threshold == "Enriched Target"), 
...
# #   theme_minimal() +
# #   scale_color_manual(values = c("Enriched Target" = "firebrick", 
# #                                 "Depleted" = "royalblue", 
# #                                 "Not Significant" = "grey80")) +
# #   labs(
# #     title = "Volcano Plot: Pulldown Enrichment",
# #     subtitle = "Labeled Pulldown vs. Unlabeled Background Control",
...
# # ggsave("Volcano_Plot_Enrichment.png", width = 8, height = 6, dpi = 300) 


# # # 1. Classify genes strictly by the DESeq2 summary criteria (padj < 0.05) ---- 
# # ma_data <- de_results_df %>%
# #   mutate(
# #     Status = case_when(
# #       log2FoldChange > 0 & padj < 0.05 ~ "Enriched Target (425 genes)",
...
# # # 2. Build the MA Plot
# # ggplot(ma_data, aes(x = baseMean, y = log2FoldChange, color = Status)) +
# #   # Plot non-significant background points
# #   geom_point(data = filter(ma_data, Status == "Not Significant"), 
# #              alpha = 0.4, size = 1.2) +
# #   # Plot depleted points
# #   geom_point(data = filter(ma_data, Status == "Depleted"), 
# #              alpha = 0.6, size = 1.8) +
# #   # Highlight the 425 true enriched targets on top
# #   geom_point(data = filter(ma_data, Status == "Enriched Target (425 genes)"), 
# #              alpha = 0.8, size = 2) +
  
# #   # Log-transform the X-axis because baseMean spans several orders of magnitude
# #   scale_x_log10(labels = scales::comma) + 
  
# #   # Add a baseline horizontal line at 0 (no change)
# #   geom_hline(yintercept = 0, linetype = "dashed", color = "grey30", linewidth = 0.6) +
  
# #   # Custom colors matching standard bioinformatics palettes
# #   scale_color_manual(values = c(
# #     "Enriched Target (425 genes)" = "firebrick3",
# #     "Depleted" = "dodgerblue3",
# #     "Not Significant" = "grey75"
# #   )) +
  
# #   # Labels and themes
# #   labs(
# #     title = "MA Plot: Labeled vs. Unlabeled Pulldown",
# #     subtitle = "Highlighting the 425 statistically significant enriched transcripts",
# #     x = "Mean of Normalized Counts (Base Expression)",
# #     y = expression(log[2] ~ "Fold Change (Pulldown Labeled / Unlabeled)"),
...
# # ggsave("MA_Plot_Enriched_Targets.png", width = 8, height = 6, dpi = 300) 


# # # ====================================================================
# # # Phase 4: Gene Ontology (GO) Enrichment Analysis of Upregulated Targets
# # # ====================================================================

# # # 1. Install and load the Mouse database if you haven't yet
# # #if (!requireNamespace("org.Mm.eg.db", quietly = TRUE)) {
# # #    BiocManager::install("org.Mm.eg.db")
# # #}
# # library(clusterProfiler)
# # library(org.Mm.eg.db) # Switched to Mouse
# # library(enrichplot)
# # library(dplyr)

# # # 2. Filter your 425 upregulated targets
# # # upregulated_genes <- de_results_df %>%
# # #   filter(log2FoldChange > 0 & padj < 0.05) %>%
# # #   pull(gene_id)

# # # 3. Convert Mouse ENSEMBL IDs to Entrez IDs
# # # Changed fromType to ENSEMBL to fix your key error!
# # gene_conversion <- bitr(
# #   upregulated_genes, 
# #   fromType = "SYMBOL",   # Change to "ENSEMBL" if your IDs look like ENSG000...
# #   toType   = "ENTREZID", 
# #   OrgDb    = org.Mm.eg.db
# # )

# # # 4. Run the Gene Ontology Enrichment Analysis (Cellular Component)
# # ego_cc <- enrichGO(
...
# # # 5. Save the full results table to a clean CSV file
# # write.csv(as.data.frame(ego_cc), "GO_Cellular_Component_Results.csv", row.names = FALSE)

# # # 6. Visualize the top 10 enriched categories
# # dotplot(ego_cc, showCategory = 10) + 
# #   labs(title = "Gene Ontology (CC) Enrichment of Upregulated Targets")

# # ggsave("GO_dotplot_upregulated.png", width = 8, height = 6, dpi = 300) 



# # ##################################################################### 
# # New variation of code 
# # ##################################################################### 
...
# # # ==========================================
# # # 1. UPDATE EXISTING DDS DESIGN & METADATA
# # # ==========================================

# # # Pull the existing metadata from your original dds object
# # meta_df <- as.data.frame(colData(dds))

# # # Parse out Fraction and Condition from your original metadata text
# # meta_df <- meta_df %>%
# #   mutate(
# #     Fraction = case_when(
# #       grepl("input", condition) ~ "input",
# #       grepl("pulldown", condition) ~ "pulldown",
# #       TRUE ~ NA_character_
# #     ),
# #     Condition_Clean = case_when(
# #       grepl("unlab", condition) ~ "unlab",
# #       grepl("lab", condition) ~ "lab",
# #       TRUE ~ NA_character_
# #     )
# #   )

# # # Combine Fraction and Condition into a unified Group variable
# # colData(dds)$Group <- factor(paste0(meta_df$Fraction, "_", meta_df$Condition_Clean))

# # # Explicitly update the design formula to use this Group variable
# # design(dds) <- ~ Group

# # ==========================================
# # 2. RUN THE DIFFERENTIAL EXPRESSION PIPELINE
# # ==========================================

# # Re-run DESeq2 using the new Group design
# dds <- DESeq(dds)

# # Apply statistical shrinkage targeted directly at your comparison of interest
# res_shrink <- lfcShrink(
#   dds, 
#   type = "ashr",
#   contrast = c("Group", "pulldown_lab", "pulldown_unlab") # Direct contrast!
# )

# # Convert to a clean dataframe
# de_results_df <- as.data.frame(res_shrink) %>%
#   rownames_to_column(var = "gene_id")

# # Strip version decimals if present (e.g., ENSMUSG000000xxxxx.13 -> ENSMUSG000000xxxxx)
# de_results_df$gene_id <- sub("\\..*", "", de_results_df$gene_id)

# # ==========================================
# # 3. RUN GENE ONTOLOGY (GO) ENRICHMENT
# # ==========================================

# # Filter strictly for significantly upregulated genes in the labeled pulldown
# upregulated_genes <- de_results_df %>%
#   filter(log2FoldChange > 0 & padj < 0.05) %>%
#   pull(gene_id)

# # Convert Ensembl IDs to Entrez IDs for clusterProfiler
# # (Note: Keep fromType = "ENSEMBL" if your IDs look like ENSMUSG0000...)
# gene_conversion <- bitr(
#   upregulated_genes, 
#   fromType = "SYMBOL",   # Change to "ENSEMBL" if your IDs look like ENSG000...
#   toType   = "ENTREZID", 
...
# # Run GO Cellular Component Analysis
# ego_cc <- enrichGO(
#   gene          = gene_conversion$ENTREZID,
#   OrgDb         = org.Mm.eg.db,
#   keyType       = "ENTREZID",
...
# # Save the data table
# write.csv(as.data.frame(ego_cc), "GO_Pulldown_Enriched_CC_Results.csv", row.names = FALSE)

# # Generate and save the dotplot
# dotplot(ego_cc, showCategory = 10) + 
#   labs(title = "GO (CC) Enrichment: Pulldown Labeled vs Unlabeled")

# ggsave("GO_pulldown_enriched_dotplot.png", width = 8, height = 6, dpi = 300) 

######## Cilium targets from here ############
